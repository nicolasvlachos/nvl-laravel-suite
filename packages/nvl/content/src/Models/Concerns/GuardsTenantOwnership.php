...
namespace Nvl\Content\Models\Concerns;
...
/** Applies Content's mandatory tenant predicate and immutable ownership stamps. */
trait GuardsTenantOwnership
{
    ...
    protected static function bootGuardsTenantOwnership(): void
    {
        ...
        static::creating(static function (Model $model): void {
            ...
            if ($definition->kind !== TenantResourceKind::Inherited || $definition->parentRelation === null) {
                return;
            }

            $relation = $model->{$definition->parentRelation}();
            if (! $relation instanceof Relation) {
                throw new TenantBoundaryViolation('Content child ownership requires a canonical relation.');
            }
            $parent = $relation->getResults();
            if (! $parent instanceof Model) {
                throw new TenantBoundaryViolation('Content child ownership requires a canonical parent.');
            }
            ...
            if (! is_string($tenantId)) {
                throw new TenantBoundaryViolation('Content child ownership requires a tenant parent.');
            }
        ...
        static::updating(static function (Model $model): void {
            if ($model->isDirty('tenant_id')) {
                throw new TenantBoundaryViolation('Persisted Content ownership is immutable.');
            }

 ...
 */
function mysqlQueuedFailureConcurrencyPrimitive(string $function): Closure
{
    if (! function_exists($function)) {
        throw new RuntimeException(sprintf(
            'The MySQL queued-failure race requires the [%s] concurrency primitive.',
            $function,
        ));
    }
 ...
 */
function runMySqlQueuedFailureWorker(
    $socket,
    string $queueReference,
): never {
    stream_set_timeout($socket, 25);
    mysqlQueuedFailureConcurrencyPrimitive('pcntl_alarm')(30);
    $exitCode = 0;
    ...
    try {
            ...
            function (string $query) use (
                ...
                if (fwrite($socket, "READY\n") === false) {
                    throw new RuntimeException(
                        'A MySQL queued-failure worker could not announce readiness.',
                    );
                }
                ...
                if (! is_float($startAt)) {
                    throw new RuntimeException(
                        'A MySQL queued-failure worker received an invalid start signal.',
                    );
                }
        ...
        if ($waitingAtInsertBoundary) {
            throw new RuntimeException(
                'A MySQL queued-failure worker did not reach the insert boundary.',
            );
        }
    ...
    mysqlQueuedFailureConcurrencyPrimitive('pcntl_alarm')(0);
...
it('fences concurrent MySQL queued-failure fallbacks and events', function () {
    if (! in_array(
        (new MailNotification)->getConnection()->getDriverName(),
        ['mysql', 'mariadb'],
        true,
    )) {
        $this->markTestSkipped(
            'The queued-failure race requires a MySQL-family connection.',
        );
    }
    ...
    if (! defined('STREAM_PF_UNIX')
        ...
        $this->markTestSkipped(
            'The MySQL queued-failure race requires Unix socket pairs.',
        );
    }
    ...
    try {
        for ($worker = 0; $worker < $workerCount; $worker++) {
            ...
            if ($pair === false) {
                $this->markTestSkipped(
                    'The MySQL queued-failure race could not create Unix socket pairs.',
                );
            }
        ...
        for ($worker = 0; $worker < $workerCount; $worker++) {
            $pid = mysqlQueuedFailureConcurrencyPrimitive('pcntl_fork')();

            if (! is_int($pid)) {
                throw new RuntimeException(
                    'The MySQL queued-failure race received an invalid worker process identifier.',
                );
            }

            if ($pid === -1) {
                $this->markTestSkipped(
                    'The MySQL queued-failure race could not fork both workers.',
                );
            }
        ...
        foreach ($parentSockets as $worker => $parentSocket) {
            ...
            if (! is_string($ready) || trim($ready) !== 'READY') {
                throw new RuntimeException(sprintf(
                    'MySQL queued-failure worker [%d] did not reach the insert boundary.',
                    $worker,
                ));
            }
        }
        ...
        foreach ($workerPids as $worker => $workerPid) {
            $status = 0;
            $waitedPid = mysqlQueuedFailureConcurrencyPrimitive(
                'pcntl_waitpid',
            )($workerPid, $status);
        ...
        foreach ($workerPayloads as $worker => $payload) {
            if ($payload === '') {
                throw new RuntimeException(sprintf(
                    'MySQL queued-failure worker [%d] returned no result.',
                    $worker,
                ));
            }
            ...
            if (! is_array($result)
                ...
                throw new RuntimeException(sprintf(
                    'MySQL queued-failure worker [%d] failed [%s].',
                    $worker,
                    $error,
                ));
            }
            ...
            $exited = is_int($status)
                ? mysqlQueuedFailureConcurrencyPrimitive(
                    'pcntl_wifexited',
                )($status)
                : false;
            ...
            if (! is_int($status)
                ...
                || $exitCode !== 0) {
                throw new RuntimeException(sprintf(
                    'MySQL queued-failure worker [%d] did not exit cleanly.',
                    $worker,
                ));
            }
            ...
            foreach (array_keys($observed) as $eventType) {
                ...
                if (! is_array($workerEvents)) {
                    throw new RuntimeException(sprintf(
                        'MySQL queued-failure worker [%d] returned invalid [%s] events.',
                        $worker,
                        $eventType,
                    ));
                }

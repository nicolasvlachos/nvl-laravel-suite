...
it('fails a representative Action for every disabled feature before database access', function (): void {
    ...
    $actions = [
        AuthFeature::Authentication->value => fn () => app(LoginAction::class)->execute($user->email, 'password'),
        AuthFeature::Password->value => fn () => app(RequestPasswordResetAction::class)->execute($user->email),
        AuthFeature::EmailVerification->value => fn () => app(RequestEmailVerificationAction::class)->execute($user),
        AuthFeature::MagicLinks->value => fn () => app(RequestMagicLinkAction::class)->execute($user->email),
        AuthFeature::SecurityCodes->value => fn () => app(RequestSecurityCodeAction::class)->execute($user->email, 'login'),
        AuthFeature::Invitations->value => fn () => app(PreviewInvitationAction::class)->execute('token'),
        AuthFeature::Totp->value => fn () => app(StartTotpEnrollmentAction::class)->execute($user, $user->email),
        ...
        AuthFeature::Clients->value => fn () => app(StartAuthClientAction::class)->execute('00000000-0000-0000-0000-000000000000', 'login', '/'),
